...
- (id)initWithVideo:(NSString *)moviePath usesTcp:(BOOL)usesTcp
{
    ...
    pCodec = avcodec_find_decoder( pCodecCtx->codec_id);       //AV_CODEC_ID_H264);   //AV_CODEC_ID_RAWVIDEO); //TODO:прямо указал кодек   //pCodecCtx->codec_id);
    int *ii = pCodecCtx->codec_id;
    NSLog(@"Code = %i", ii);
    if (pCodec == NULL) {
    ...
    NSLog(@"videoHeight = %i", videoHeight);
    
    self.outputWidth = pCodecCtx->width;
    int *videoWidth = pCodecCtx->width;
    NSLog(@"videoWidth = %i", videoWidth);
